    ...
    <header className="sticky top-0 z-40 border-b border-black/[0.06] bg-[#F5F5F7]/80 backdrop-blur-xl supports-[backdrop-filter]:bg-[#F5F5F7]/75 transition-all duration-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 gap-4">
          ...
          >
            <div className="w-8 h-8 rounded-xl bg-[#0071E3] flex items-center justify-center shadow-sm group-hover:scale-105 transition-transform duration-200">
          ...
          <div className="hidden lg:flex items-center space-x-2.5 shrink-0">
            ...
            <div className="flex items-center space-x-2 px-3 py-1.5 rounded-full bg-white border border-black/[0.06] text-xs font-medium text-[#6E6E73] shadow-sm">
              <span className="w-2 h-2 rounded-full bg-[#34C759]"></span>
              <span>{totalAccounts.toLocaleString()} AD Accounts</span>
            </div>
          </div>

          {/* ZONE 3: NAVIGATION TABS - APPLE SEGMENTED CONTROL */}
          <nav className="flex items-center p-1 rounded-xl bg-[#E5E5EA]/80 border border-black/[0.04] overflow-x-auto no-scrollbar">
            {navItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium transition-all duration-200 shrink-0 ${
                    isActive
                      ? "bg-white text-[#1D1D1F] shadow-sm font-semibold"
                      : "text-[#6E6E73] hover:text-[#1D1D1F] hover:bg-black/[0.03]"
                  }`}
                >
                  <Icon className={`w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0 ${isActive ? "text-[#0071E3]" : "text-[#86868B]"}`} />
